...
case $1 in
    ...
    "init")
        if [ -f ~/.cache/current_wallpaper.jpg ]; then
            ...
            wal -q -i ~/.config/hypr/wallpaper/
        fi
    ;;

   # Select wallpaper with rofi
    "select")
        selected=$(ls -1 ~/.config/hypr/wallpaper/LightWallpapers/ ~/.config/hypr/wallpaper/DarkWallpapers/ | rofi -dmenu -replace -config ~/.config/rofi/config-wallpaper.rasi)
        if [ ! "$selected" ]; then
            echo "No wallpaper selected"
            exit
        fi
        if [[ -f ~/.config/hypr/wallpaper/LightWallpapers/$selected ]]; then
            wallpaper=~/.config/hypr/wallpaper/LightWallpapers/$selected
            theme="light"
            wal -l -q -i "$wallpaper" # Use the -l flag for light wallpapers
        elif [[ -f ~/.config/hypr/wallpaper/DarkWallpapers/$selected ]]; then
            wallpaper=~/.config/hypr/wallpaper/DarkWallpapers/$selected
            theme="dark"
            wal -q -i "$wallpaper" 
        else
